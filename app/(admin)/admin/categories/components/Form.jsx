      ...
      >
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="category-parent" className="text-gray-500 text-sm">
            Parent Category <span className="text-gray-400">(Optional - for subcategories)</span>
          </label>
          <select
            id="category-parent"
            ...
            value={data?.parentId ?? ""}
            onChange={(e) => {
              handleData("parentId", e.target.value || null);
            }}
            className="border px-4 py-2 rounded-lg w-full focus:outline-none"
          >
            <option value="">None (Main Category)</option>
            {categories
              ?.filter((cat) => !cat.parentId && cat.id !== id) // Only show main categories and exclude self
              ?.map((category) => (
