...
export default function ProductsGridView({
  ...
  return (
    <section className="w-full flex justify-center bg-background py-10">
      <div className="flex flex-col gap-8 max-w-[1500px] p-5 w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-5 w-full">
          {products?.map?.((item) => (
            <ProductCard
              product={item}
              key={item?.id}
...
export function ProductCard({ product, className, fromSlider }) {
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col gap-3 border-2 border-pink-50 bg-white rounded-3xl overflow-hidden hover:shadow-xl hover:border-theme-pink/30 transition-all duration-300 ${className}`}
    >
      {/* Media area */}
      <div className="relative w-full bg-pink-50 aspect-square">
      ...
      {/* Meta */}
      <div className="flex flex-col justify-between flex-grow gap-2 p-3">
        <div className="flex flex-col gap-1">
          <h3 className="font-fredoka font-bold text-sm md:text-base text-gray-800 line-clamp-2 leading-tight">
            {product?.title}
          </h3>

          {/* Price */}
          <div className="flex items-center gap-2 mt-1">
            <span className="text-theme-darkPink text-sm md:text-base font-bold font-quicksand">
              Rs {product?.salePrice}
            </span>
            {product?.salePrice < product?.price && (
              <span className="line-through text-xs text-gray-400 font-quicksand">
                Rs {product?.price}
              </span>
            )}
          </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-none py-1">
              {selectableSizes.slice(0, 5).map((s) => (
                <button
                  ...
                  className={`shrink-0 inline-flex items-center justify-center rounded-lg border px-2 py-1 text-[10px] font-quicksand font-bold ${selectedSize === s
                    ? "border-theme-pink bg-theme-pink text-white"
                    : "border-theme-light text-gray-500 hover:border-theme-pink/50"
                    }`}
                >
                  {s}
                </button>
        ...
        {!isOutOfStock && (
          <div className="flex items-center gap-3 w-full mt-3">
            <button
              ...
              className="flex-1 glossy-btn text-white py-1.5 rounded-xl text-xs md:text-sm font-bold font-fredoka tracking-wide"
