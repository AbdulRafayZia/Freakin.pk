...
export const searchAndFilterProducts = async (params) => {
  ...
  if (params.category && params.category.length > 0) {
    q = query(q, where("categoryId", "in", params.category));
  }
  ...
  // Additional product type filters (e.g., Best Selling, New Arrivals)
  // would need logic on how to define them, possibly with dedicated flags or different queries
  ...
  if (params.color && params.color.length > 0) {
    q = query(
      q,
      where("variants.color.name", "array-contains-any", params.color)
    );
  }
  ...
  // Move out-of-stock products to the end of the array
  products = [
    ...products.filter(
      (product) => (product?.stock ?? 0) > (product?.orders ?? 0)
    ),
    ...products.filter(
      (product) => (product?.stock ?? 0) <= (product?.orders ?? 0)
    ),
